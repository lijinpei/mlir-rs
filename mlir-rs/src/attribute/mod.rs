...
use crate::support::*;
use mlir_capi::IR::*;
use mlir_capi::{BuiltinAttributes, IR};
...
impl<'ctx> Attr<'ctx> {
    pub fn parse(ctx: &'ctx Context, attr: &str) -> Self {
        unsafe { IR::FFIVal_::mlirAttributeParseGet(ctx, StrRef::from(attr)) }
    }
    pub fn get_context(self) -> ContextRef<'ctx> {
        let handle = unsafe { IR::FFIVal_::mlirAttributeGetContext(self) };
        unsafe { ContextRef::construct(handle, self.phantom) }
    }
    pub fn get_type(self) -> Type<'ctx> {
        unsafe { IR::FFIVal_::mlirAttributeGetType(self) }
    }
    pub fn get_typeid(self) -> TypeID {
        unsafe { IR::FFIVal_::mlirAttributeGetTypeID(self) }
    }
    pub fn get_dialect(self) -> Dialect<'ctx> {
        unsafe { IR::FFIVal_::mlirAttributeGetDialect(self) }
    }
    pub fn is_null(self) -> bool {
        to_rbool(unsafe { IR::FFIVal_::mlirAttributeIsNull(self) })
    }
