...
// FIXME: makes this a trait
impl<'ctx> ElementsAttr<'ctx> {
    pub fn get_value(self, indices: &[u64]) -> Attr<'ctx> {
        let handle = unsafe {
    ...
    pub fn is_valid_index(self, indices: &[u64]) -> bool {
        to_rbool(unsafe {
...
// FIXME: get typeid
pub trait DenseArrrayAttr<'ctx>: Into<MlirAttribute> {
...
impl<'ctx> DenseBoolArrayAttr<'ctx> {
    pub fn get(ctx: &'ctx Context, values: &[CBool]) -> Self {
        let handle = unsafe {
