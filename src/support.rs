...
impl PartialEq<TypeID> for TypeID {
    fn eq(&self, other: &TypeID) -> bool {
        (unsafe { mlir_capi::Support::FFIVal_::<u8>::mlirTypeIDEqual(self, other) }) != 0
    }
}
...
impl<'a> Into<&'a str> for &StrRef<'a> {
    fn into(self) -> &'a str {
        unsafe {
            let slice = std::slice::from_raw_parts(self.handle.data as _, self.handle.length as _);
...
// FIXME: why Eq takes no generic parameter?
